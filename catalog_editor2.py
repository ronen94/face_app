...
class CatalogEditor:
    ...
    def init_feature_catalog(self):
        ...
        if mustahce_folder.exists():
            for img_file in mustahce_folder.glob("*.png"):
                try:
                    # Use filename without extension as the label
                    name = img_file.stem.replace("_", " ").title()
                    img = Image.open(img_file).convert("RGBA")
                    ...
                    self.feature_catalog['mustache'][name] = img
                    print(f"✓ Loaded: {name} from {img_file.name}")
                except Exception as e:
                    print(f"✗ Failed to load {img_file.name}: {e}")
    ...
    def change_category(self, category):
        """Handle category change while preserving the current image"""
        gallery = self.create_catalog_gallery(category)
        current_img = self.get_current_display_image()  # Preserve the current image
        return current_img, gallery
    ...
    def select_from_catalog(self, evt: gr.SelectData, category):
        ...
        return (
            current_img,  # Return current image to keep it displayed
            f"✓ Selected: {selected_name} from {category}\n💡 Adjust size/rotation/opacity below and watch the preview update!",
            preview_img,
    ...
    def handle_image_upload(self, img):
        ...
        self.overlays = []
        self.preview_overlay = None
        return np.array(self.base_image), "✓ Image loaded! Now select a feature from the catalog and click on the image to place it."
    ...
    def handle_image_click(self, img, evt: gr.SelectData):
        ...
        if self.preview_overlay is not None:
            ...
            return np.array(result), f"🔄 Moved {feature_name} to ({click_x}, {click_y}). Click 'Confirm' or click again to adjust."
        else:
            ...
            result = self.composite_image()
            return np.array(result), f"✓ Preview: {feature_name} at ({click_x}, {click_y}). Click 'Confirm' to keep it, or click again to move it."
    ...
    def update_scale(self, scale):
        ...
        preview_img = self.get_feature_preview()
        # IMPORTANT: Always return current composited image (if available) so Gradio doesn't clear it.
        ...
        # Even if there's no preview, return the current image so the display remains intact
        return current_img, f"Scale: {scale:.2f}x", preview_img
...
def create_interface():
    ...
    with gr.Blocks(title="Feature Catalog Editor", theme=gr.themes.Soft()) as interface:
        ...
        with gr.Row():
            ...
            with gr.Column(scale=1):
                ...
                category_select = gr.Radio(
                    choices=["eyes", "beard", "mustache"],  # Add more as needed
                    value="eyes",
        ...
        # Update catalog when category changes - now preserves image
        ...
        # Select from catalog - now preserves image and updates preview
